...
namespace PapyrusVR
{
    ...
    Matrix34 operator+(Matrix34 const& lhs, Matrix34 const& rhs)
    {
        Matrix34 result;
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 4; j++) result.m[i][j] = lhs.m[i][j] + rhs.m[i][j];
        return result;
    }
    ...
    {
        Matrix33 result = Matrix33(matrix->m[0][0], matrix->m[0][1], matrix->m[0][2], matrix->m[1][0], matrix->m[1][1], matrix->m[1][2], matrix->m[2][0],
    ...
    {
        Matrix34 result = Matrix34(matrix->m[0][0], matrix->m[0][1], matrix->m[0][2], 0, matrix->m[1][0], matrix->m[1][1], matrix->m[1][2], 0, matrix->m[2][0],
                                   matrix->m[2][1], matrix->m[2][2], 0);
        return result;
    }
}